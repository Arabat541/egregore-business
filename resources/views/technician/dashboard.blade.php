...
<!-- Chiffre d'affaires réparations (part technicien {{ $stats['tech_share_percent'] }}%) -->
<div class="row g-4 mb-4">
    <div class="col-md-6">
        <div class="card border-success">
            <div class="card-body text-center">
                <h5 class="text-muted mb-1"><i class="bi bi-calendar-day"></i> Ma part Jour <span class="badge bg-secondary">{{ $stats['tech_share_percent'] }}%</span></h5>
                <h2 class="text-success mb-0">{{ number_format($stats['ca_day'] ?? 0, 0, ',', ' ') }} FCFA</h2>
                @if(($stats['sav_deduction_day'] ?? 0) > 0)
                    <a href="{{ route('technician.sav-deductions') }}" class="text-danger text-decoration-none">
                        <small><i class="bi bi-arrow-down"></i> -{{ number_format($stats['sav_deduction_day'], 0, ',', ' ') }} F (SAV)</small>
                    </a>
                @endif
            </div>
        </div>
    ...
    <div class="col-md-6">
        <div class="card border-primary">
            <div class="card-body text-center">
                <h5 class="text-muted mb-1"><i class="bi bi-calendar-month"></i> Ma part Mois <span class="badge bg-secondary">{{ $stats['tech_share_percent'] }}%</span></h5>
                <h2 class="text-primary mb-0">{{ number_format($stats['ca_month'] ?? 0, 0, ',', ' ') }} FCFA</h2>
                @if(($stats['sav_deduction_month'] ?? 0) > 0)
                    <a href="{{ route('technician.sav-deductions') }}" class="text-danger text-decoration-none">
                        <small><i class="bi bi-arrow-down"></i> -{{ number_format($stats['sav_deduction_month'], 0, ',', ' ') }} F (SAV)</small>
                    </a>
                @endif
            </div>
        </div>
...
<div class="card mt-4">
    <div class="card-header bg-success text-white">
        <i class="bi bi-check-circle"></i> Réparations terminées & livrées
    </div>
    <div class="card-body">
        @if($deliveredRepairs->count() > 0)
                ...
                <table class="table table-sm">
                    <thead>
                        <tr>
                            ...
                            <th>Appareil</th>
                            <th>Main d'œuvre</th>
                            <th>Ma part ({{ $stats['tech_share_percent'] }}%)</th>
                            <th>Statut</th>
                            <th>Date</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            <td><a href="{{ route('technician.repairs.show', $repair) }}"><code>{{ $repair->repair_number }}</code></a></td>
                            <td>{{ $repair->customer->full_name ?? '-' }}</td>
                            <td>{{ $repair->device_brand }} {{ $repair->device_model }}</td>
                            <td>{{ number_format($repair->labor_cost, 0, ',', ' ') }} F</td>
                            <td class="text-success fw-bold">{{ number_format($repair->labor_cost * $stats['tech_share_percent'] / 100, 0, ',', ' ') }} F</td>
                            <td><span class="badge bg-{{ $repair->status_color }}">{{ $repair->status_label }}</span></td>
                            <td>{{ $repair->repaired_at ? $repair->repaired_at->format('d/m/Y') : '-' }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="text-center">
                <a href="{{ route('technician.repairs.index') }}" class="btn btn-outline-success btn-sm">
                    <i class="bi bi-list"></i> Voir tout l'historique
                </a>
            </div>
        @else
