    ...
    <div class="col-md-6">
        ...
        <div class="card mb-3">
            <div class="card-header d-flex justify-content-between align-items-center">
                <span><i class="bi bi-shield-lock"></i> Double authentification (Google Authenticator)</span>
            ...
            <div class="card-body">
                @if($user->two_factor_enabled)
                    <p class="text-muted small mb-3">
                        ...
                        Votre compte est protégé par Google Authenticator. Un code TOTP est requis à chaque connexion.
                    </p>
                    {{-- Désactivation avec confirmation du mot de passe --}}
                    <button class="btn btn-sm btn-outline-danger" data-bs-toggle="collapse" data-bs-target="#disable2fa">
                    ...
                    <div class="collapse mt-3" id="disable2fa">
                        <div class="card card-body bg-light border-danger border-opacity-25">
                            <p class="small text-danger mb-2"><i class="bi bi-exclamation-triangle me-1"></i>Confirmez votre mot de passe pour désactiver.</p>
                            <form action="{{ route('profile.2fa.disable') }}" method="POST" class="d-flex gap-2">
                                @csrf
                                ...
                                @error('password')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <button type="submit" class="btn btn-sm btn-danger text-nowrap">Confirmer</button>
                            </form>
                        </div>
                    </div>
                @else
                    <p class="text-muted small mb-3">
                        Protégez votre compte avec l'application <strong>Google Authenticator</strong>.
                        À chaque connexion, un code à 6 chiffres sera demandé depuis votre téléphone.
                    </p>
                    <a href="{{ route('profile.2fa.setup') }}" class="btn btn-sm btn-outline-primary">
                        <i class="bi bi-shield-check"></i> Configurer Google Authenticator
                    </a>
                @endif
            </div>
        </div>
