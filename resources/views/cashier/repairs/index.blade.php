...
<div class="card">
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                ...
                                @if($repair->amount_paid > 0)
                                    <br><small class="text-success">Avance: {{ number_format($repair->amount_paid, 0, ',', ' ') }} F</small>
                                @endif
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    ...
                                    <a href="{{ route('cashier.repairs.edit', $repair) }}" class="btn btn-outline-warning" title="Modifier">
                                    ...
                                    @if(!in_array($repair->status, ['delivered', 'cancelled']))
                                        <button type="button" class="btn btn-outline-danger" title="Annuler"
                                                data-bs-toggle="modal" data-bs-target="#cancelModal{{ $repair->id }}">
                        ...
                        <tr>
                            <td colspan="9" class="p-0">
                                <x-empty-state
                                    ...
                                    :message="request()->hasAny(['search','status','technician','date_from','date_to'])
                                        ? 'Aucun résultat pour ces filtres. Modifiez ou supprimez les critères.'
                                        : 'Il n\'y a pas encore de réparation enregistrée.'"
    ...
    <div class="card-footer d-flex justify-content-between align-items-center flex-wrap gap-2">
        <div class="text-muted small">
            {{ $repairs->firstItem() }}–{{ $repairs->lastItem() }} sur {{ $repairs->total() }} réparations
        </div>
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <h4>{{ number_format($repair->final_cost ?? $repair->estimated_cost ?? 0, 0, ',', ' ') }} F</h4>
                            </div>
                            <div class="col-6">
                                <p><strong>Avance payée (pièces):</strong></p>
                                <h4 class="text-success">{{ number_format($repair->amount_paid ?? 0, 0, ',', ' ') }} F</h4>
                            </div>
                        </div>
                        @php $remainingAmount = ($repair->final_cost ?? $repair->estimated_cost ?? 0) - ($repair->amount_paid ?? 0); @endphp
                        @if($remainingAmount > 0)
                            <div class="alert alert-warning">
                                <strong>Reste à payer (main d'œuvre):</strong> 
                            ...
                            <div class="mb-3">
                                ...
                                <input type="number" name="paid_amount" class="form-control" 
                                       value="{{ $remainingAmount }}" min="0" required>
                            </div>
                        @else
...
@foreach($repairs as $repair)
    @if(!in_array($repair->status, ['delivered', 'cancelled']))
    <div class="modal fade" id="cancelModal{{ $repair->id }}" tabindex="-1">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content border-danger">
                <div class="modal-header bg-danger text-white">
                    <h5 class="modal-title">
                        <i class="bi bi-x-circle me-2"></i>Annuler — {{ $repair->repair_number }}
                    </h5>
                    <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                        ...
                        <div class="alert alert-warning d-flex gap-2 mb-3">
                            ...
                            <div>
                                ...
                                @if($repair->parts->count() > 0)
                                    <br><strong>{{ $repair->parts->count() }} pièce(s)</strong> seront remise(s) en stock.
                                @endif
                                @if($repair->amount_paid > 0)
                                    <br>Acompte de <strong>{{ number_format($repair->amount_paid, 0, ',', ' ') }} F</strong> à rembourser manuellement.
                                @endif
                            </div>
                        </div>
                        <div class="mb-2 small text-muted">
                            {{ $repair->device_brand }} {{ $repair->device_model }} — {{ $repair->customer->full_name ?? '' }}
                        </div>
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" name="cancel_reason" rows="3"
                                      placeholder="Ex: Client a annulé, pièce introuvable..."
