...
<div class="row g-4 mb-4">
    <div class="col-12">
        <div class="card border-0 shadow-sm" style="background: linear-gradient(135deg, #0d6efd, #6610f2);">
            <div class="card-body py-4">
                <div class="row align-items-center">
                    <div class="col-md-4 text-center text-white border-end border-white border-opacity-25">
                        <i class="bi bi-graph-up-arrow" style="font-size: 2.5rem;"></i>
                        <h6 class="text-white-50 mt-2 mb-1">CA Total du jour</h6>
                        <h2 class="mb-0 fw-bold">{{ number_format($stats['today_total_ca'], 0, ',', ' ') }} <small class="fs-6">F</small></h2>
                    </div>
                    <div class="col-md-4 text-center text-white border-end border-white border-opacity-25">
                        <i class="bi bi-cart-check" style="font-size: 1.8rem;"></i>
                        <h6 class="text-white-50 mt-1 mb-1">Ventes</h6>
                        <h3 class="mb-0">{{ number_format($stats['today_sales_amount'], 0, ',', ' ') }} <small class="fs-6">F</small></h3>
                        <small class="text-white-50">
                            {{ $stats['today_sales_count'] }} vente(s){{ ($stats['today_repairs_with_parts'] ?? 0) > 0 ? ' + ' . $stats['today_repairs_with_parts'] . ' pièce(s) de rechange' : '' }}
                        </small>
                    </div>
                    <div class="col-md-4 text-center text-white">
                        <i class="bi bi-tools" style="font-size: 1.8rem;"></i>
                        <h6 class="text-white-50 mt-1 mb-1">Réparations</h6>
                        <h3 class="mb-0">{{ number_format($stats['today_repairs_amount'], 0, ',', ' ') }} <small class="fs-6">F</small></h3>
                        <small class="text-white-50">{{ $stats['today_repairs_count'] }} fiche(s)</small>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-md-3">
        <div class="card stat-card">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <div>
                        <h6 class="text-muted">Carte bancaire</h6>
                        <h3 class="mb-0">{{ number_format($stats['today_card_sales'], 0, ',', ' ') }}</h3>
                    </div>
    ...
    <div class="col-md-3">
        <div class="card stat-card warning">
            <div class="card-body">
                <div class="d-flex justify-content-between">
                    <div>
                        <h6 class="text-muted">Crédit</h6>
                        <h3 class="mb-0">{{ number_format($stats['today_credit_sales'], 0, ',', ' ') }}</h3>
                    </div>
...
<div class="card mt-4">
    <div class="card-header bg-info text-white">
        <i class="bi bi-tools"></i> Réparations terminées & livrées
    </div>
    <div class="card-body">
        @if($deliveredRepairs->count() > 0)
            <div class="table-responsive">
                <table class="table table-sm">
                    <thead>
                        <tr>
                    ...
                    <tbody>
                        @foreach($deliveredRepairs as $repair)
                        <tr>
                            <td><a href="{{ route('cashier.repairs.show', $repair) }}"><code>{{ $repair->repair_number }}</code></a></td>
                            <td>{{ $repair->customer->full_name ?? '-' }}</td>
                            <td>{{ $repair->device_brand }} {{ $repair->device_model }}</td>
                            <td>{{ $repair->technician->name ?? '-' }}</td>
                            <td>{{ number_format($repair->labor_cost, 0, ',', ' ') }} F</td>
                            <td>{{ number_format($repair->parts_cost ?? 0, 0, ',', ' ') }} F</td>
                            <td><span class="badge bg-{{ $repair->status_color }}">{{ $repair->status_label }}</span></td>
                            <td>{{ $repair->delivered_at ? $repair->delivered_at->format('d/m/Y') : ($repair->repaired_at ? $repair->repaired_at->format('d/m/Y') : '-') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
        ...
        @else
            <p class="text-muted text-center mb-0">Aucune réparation terminée</p>
        @endif
    </div>
</div>

{{-- Widget : Transferts entrants en attente --}}
@if($pendingTransfers->isNotEmpty())
<div class="card mt-4 border-warning">
    <div class="card-header bg-warning text-dark d-flex justify-content-between align-items-center">
        <span><i class="bi bi-truck me-2"></i><strong>Transferts en attente de réception</strong></span>
        <span class="badge bg-danger">{{ $pendingTransfers->count() }} à confirmer</span>
    </div>
            ...
            <table class="table table-sm mb-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><i class="bi bi-shop text-primary"></i> {{ $transfer->fromShop->name }}</td>
                        <td class="text-center">{{ $transfer->total_items }} article(s)</td>
                        <td>{{ $transfer->in_transit_at?->format('d/m/Y H:i') ?? $transfer->created_at->format('d/m/Y') }}</td>
                        <td class="text-center">
                            <a href="{{ route('cashier.stock-transfers.show', $transfer) }}"
                               class="btn btn-sm btn-success">
                                <i class="bi bi-box-arrow-in-down me-1"></i>Confirmer
                            </a>
                        </td>
                    </tr>
                    @endforeach
