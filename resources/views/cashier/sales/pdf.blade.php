...
.kpi { display: table-cell; width: 25%; background: #f8f9fa; border: 1px solid #dee2e6; border-radius: 4px; padding: 5px 8px; text-align: center; }
...
.badge-credit { color: #dc3545; font-weight: bold; }
...
<div class="header">
    ...
    <div class="header-right">
        ...
        @endif
        <div class="badge">{{ $sales->count() }} ventes</div><br>
...
<table>
    <thead>
        <tr>
            ...
            <th style="width:8%">Type client</th>
            <th style="width:22%">Articles</th>
            <th class="num" style="width:9%">Total (F)</th>
            <th class="num" style="width:9%">Payé (F)</th>
            <th style="width:8%">Mode paie.</th>
    ...
    <tr>
        ...
        <td class="{{ $sale->payment_status === 'paid' ? 'badge-paid' : 'badge-credit' }}">
            {{ $sale->payment_status === 'paid' ? 'Payé' : 'Crédit' }}
        </td>
        <td>{{ $sale->user->name ?? '—' }}</td>
    </tr>
    @empty
    <tr><td colspan="10" style="text-align:center;color:#888;padding:12px">Aucune vente trouvée.</td></tr>
    @endforelse
    </tbody>
    <tfoot>
        <tr>
            <td colspan="5">TOTAL ({{ $sales->count() }} ventes)</td>
