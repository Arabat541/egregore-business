        ...
        <div class="col-md-4">
            <div class="card sticky-top" style="top: 20px;">
                ...
                <div class="card-body">
                    <!-- Remise en devise -->
                    <div class="mb-2">
                        <label class="form-label form-label-sm mb-1">Remise (FCFA)</label>
                    ...
                    <div class="d-flex justify-content-between mb-1" id="subtotalRow" style="display:none!important">
                        ...
                        <span class="text-muted" id="subtotalAmount">0 FCFA</span>
                    </div>
...
function getPriceForQuantity(product, quantity, clientType = null) {
    ...
    // Réparateur: grille de prix dégressive
    const qtyWholesale     = product.qty_wholesale_min      || 10;
    const qtySemiWholesale = product.qty_semi_wholesale_min || 3;
    ...
    // En-dessous du seuil demi-gros = prix réparateur
    return parseFloat(product.reseller_price);
}

function getPriceTypeLabel(quantity, product = null, clientType = null) {
    const type = clientType || document.getElementById('clientType').value;
    if (type !== 'reseller') return '';
    const qtyWholesale     = (product && product.qty_wholesale_min)      || 10;
    const qtySemiWholesale = (product && product.qty_semi_wholesale_min) || 3;
    if (quantity >= qtyWholesale)     return '(gros)';
    if (quantity >= qtySemiWholesale) return '(demi-gros)';
    return '(répar.)';
}
...
function renderCart() {
    ...
    if (cart.length === 0) {
        ...
        tbody.innerHTML = cart.map((item, i) => {
            ...
            const priceLabel = getPriceTypeLabel(item.quantity, item);
            return `
            <tr>
                <td>
                    ${item.name}
...
function calculateTotals() {
    ...
    // La remise ne peut pas être >= au sous-total
    if (subtotal > 0 && discountAmount >= subtotal) {
        const maxDiscount = subtotal - 1;
        discountInput.value = maxDiscount;
        discountAmount = maxDiscount;
        discountError.textContent = 'La remise ne peut pas être supérieure ou égale au montant total.';
    ...
    // Afficher/masquer la ligne remise
    const discountRow = document.getElementById('discountRow');
    const subtotalRow = document.getElementById('subtotalRow');
    if (effectiveDiscount > 0) {
        discountRow.style.display = 'flex';
        subtotalRow.style.removeProperty('display');
        document.getElementById('subtotalAmount').textContent = formatNumber(subtotal) + ' FCFA';
        document.getElementById('discountDisplay').textContent = '- ' + formatNumber(effectiveDiscount) + ' FCFA';
    } else {
