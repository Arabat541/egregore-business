...
<a href="{{ route('cashier.pending-sales.index') }}" class="nav-link {{ request()->routeIs('cashier.pending-sales.*') ? 'active' : '' }}">
    <i class="bi bi-clock-history"></i> Ventes en attente
</a>
...
<a href="{{ route('cashier.resellers.index') }}" class="nav-link {{ request()->routeIs('cashier.resellers.*') ? 'active' : '' }}">
...
@php
    $incomingCount = \App\Models\StockTransfer::where('to_shop_id', auth()->user()->shop_id)
        ->where('status', 'in_transit')->count();
@endphp
<a href="{{ route('cashier.stock-transfers.index') }}" class="nav-link {{ request()->routeIs('cashier.stock-transfers.*') ? 'active' : '' }}">
    <i class="bi bi-box-arrow-in-down"></i> Transferts entrants
    @if($incomingCount > 0)
        <span class="badge bg-danger ms-1">{{ $incomingCount }}</span>
    @endif
</a>
