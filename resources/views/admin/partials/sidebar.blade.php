...
{{-- Boutiques --}}
@php
    $shopsActive = request()->routeIs('admin.shops.*') || request()->routeIs('admin.online-orders.*');
    $shopsCount = \App\Models\Shop::active()->count();
    $pendingOnlineOrders = \App\Models\OnlineOrder::where('status', 'pending')->count();
@endphp
<div class="sidebar-menu-group">
    ...
    <div class="collapse {{ $shopsActive ? 'show' : '' }}" id="menuBoutiques">
        <div class="sidebar-submenu">
            ...
            <a href="{{ route('admin.online-orders.index') }}" class="nav-link {{ request()->routeIs('admin.online-orders.*') ? 'active' : '' }}">
                <i class="bi bi-globe"></i> Boutique en ligne
                @if($pendingOnlineOrders > 0)
                    <span class="badge bg-warning ms-1">{{ $pendingOnlineOrders }}</span>
                @endif
            </a>
        </div>
    </div>
...
{{-- Gestion Stock --}}
@php
    $stockActive = request()->routeIs('admin.categories.*') || request()->routeIs('admin.products.*') || request()->routeIs('admin.suppliers.*') || request()->routeIs('admin.inventory.*') || request()->routeIs('admin.stock-movements.*') || request()->routeIs('admin.stock-transfers.*');
    ...
    $inProgressInventory = \App\Models\Inventory::whereIn('status', ['in_progress', 'completed'])->count();
@endphp
<div class="sidebar-menu-group">
    <button class="sidebar-menu-header {{ $stockActive ? 'active' : '' }}" 
        ...
        <span>
            @if($lowStockCount > 0 || $pendingTransfers > 0)
                <span class="badge bg-warning me-1">{{ $lowStockCount + $pendingTransfers }}</span>
            @endif
            <i class="bi bi-chevron-right chevron"></i>
        </span>
    </button>
    <div class="collapse {{ $stockActive ? 'show' : '' }}" id="menuStock">
        <div class="sidebar-submenu">
            ...
            <a href="{{ route('admin.inventory.index') }}" class="nav-link {{ request()->routeIs('admin.inventory.*') ? 'active' : '' }}">
                ...
                @if($inProgressInventory > 0)
                    <span class="badge bg-info ms-1">{{ $inProgressInventory }}</span>
                @endif
            </a>
            ...
            <a href="{{ route('admin.suppliers.orders') }}" class="nav-link {{ request()->routeIs('admin.suppliers.orders*') ? 'active' : '' }}">
                <i class="bi bi-file-earmark-text"></i> Factures fournisseurs
            </a>
            <a href="{{ route('admin.suppliers.low-stock') }}" class="nav-link {{ request()->routeIs('admin.suppliers.low-stock') ? 'active' : '' }}">
                <i class="bi bi-cart-plus"></i> À commander
            </a>
    ...
    <div class="collapse {{ $financeActive ? 'show' : '' }}" id="menuFinance">
        <div class="sidebar-submenu">
            ...
            <a href="{{ route('admin.resellers.index') }}" class="nav-link {{ request()->routeIs('admin.resellers.index', 'admin.resellers.show', 'admin.resellers.create', 'admin.resellers.edit', 'admin.resellers.statement') ? 'active' : '' }}">
                <i class="bi bi-person-badge"></i> Réparateurs
            </a>
