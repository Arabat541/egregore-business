...
@if($order->status === 'received')
    @php
        $hasDiscrepancy = $order->items->contains(fn($i) => $i->quantity_received != $i->quantity_ordered && $i->quantity_received !== null);
    @endphp
    @if($hasDiscrepancy)
    <div class="alert alert-warning border-warning mb-4">
        <div class="d-flex align-items-start">
            <i class="bi bi-exclamation-triangle-fill fs-4 me-3 text-warning flex-shrink-0"></i>
            <div>
                <strong>Écart de réception</strong><br>
                Certaines quantités reçues ne correspondent pas aux quantités commandées.
                Les stocks ont été mis à jour avec les quantités réellement reçues.
                @if($order->reception_notes)
                    <br><strong>Note de réception :</strong> {{ $order->reception_notes }}
                @endif
            </div>
        </div>
    </div>
    @elseif($order->reception_notes)
    <div class="alert alert-info mb-4">
        <i class="bi bi-chat-text me-2"></i><strong>Note de réception :</strong> {{ $order->reception_notes }}
    </div>
    @endif
@endif
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @foreach($order->items as $item)
                                @php
                                    $itemDiscrepancy = $order->status === 'received'
                                        && $item->quantity_received !== null
                                        && $item->quantity_received != $item->quantity_ordered;
                                @endphp
                                <tr class="{{ $itemDiscrepancy ? 'table-warning' : '' }}">
                                    <td>
                                        @if($item->product)
                                    ...
                                    <td class="text-center">
                                        @if($item->quantity_received == $item->quantity_ordered)
                                            <span class="text-success fw-bold"><i class="bi bi-check-circle"></i> {{ $item->quantity_received }}</span>
                                        @elseif($item->quantity_received < $item->quantity_ordered)
                                            <span class="text-danger fw-bold">{{ $item->quantity_received }}</span>
                                            <small class="text-danger d-block">-{{ $item->quantity_ordered - $item->quantity_received }} manquant(s)</small>
                                        @else
                                            <span class="text-warning fw-bold">{{ $item->quantity_received }}</span>
                                            <small class="text-success d-block">+{{ $item->quantity_received - $item->quantity_ordered }} en plus</small>
                                        @endif
                                    </td>
                                    @endif
            ...
            <form action="{{ route('admin.suppliers.orders.receive', $order) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        ...
                        Saisissez les quantités <strong>réellement reçues</strong> et confirmez les prix.
                        Les stocks seront mis à jour et tout écart sera signalé.
                    </div>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $loop->index }}][quantity_received]"
                                                   class="form-control form-control-sm text-center rcv-qty"
                                                   value="{{ $item->quantity_ordered }}"
                                                   min="0"
                                                   data-ordered="{{ $item->quantity_ordered }}"
                                                   required>
                                        </td>
                                        <td class="text-center rcv-ecart">—</td>
                                        <td>
                    ...
                    <div class="mb-0">
                        <label class="form-label fw-bold">
                            ...
                            <small class="text-muted fw-normal">(optionnel — décrivez tout problème, écart ou remarque)</small>
                        </label>
                        <textarea name="reception_notes" class="form-control" rows="2"
                                  placeholder="Ex: 2 boîtes abîmées, prix différent sur facture fournisseur, livraison incomplète..."></textarea>
                    </div>
                </div>
...
@push('scripts')
<script>
document.querySelectorAll('.rcv-qty').forEach(function(input) {
    input.addEventListener('input', function() {
        const ordered = parseInt(this.dataset.ordered, 10);
        const received = parseInt(this.value, 10);
        const cell = this.closest('tr').querySelector('.rcv-ecart');
        if (isNaN(received) || isNaN(ordered)) {
            cell.innerHTML = '—';
            return;
        }
        const diff = received - ordered;
        if (diff === 0) {
            cell.innerHTML = '<span class="text-success fw-bold"><i class="bi bi-check-circle"></i> 0</span>';
        } else if (diff < 0) {
            cell.innerHTML = '<span class="text-danger fw-bold">' + diff + '</span>';
        } else {
            cell.innerHTML = '<span class="text-warning fw-bold">+' + diff + '</span>';
        }
    });
});
</script>
