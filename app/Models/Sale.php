...
class Sale extends Model
{
    ...
    public static function generateInvoiceNumber(?int $shopId = null): string
    {
        ...
        // Rechercher la dernière vente du jour pour cette boutique
        $query = self::whereDate('created_at', today());
        if ($shopId) {
            $query->where('shop_id', $shopId);
        }
        
        $lastSale = $query->orderBy('id', 'desc')->first();
        $sequence = 1;
        ...
        // Format avec shop_id pour éviter les conflits entre boutiques
        $shopPrefix = $shopId ? "S{$shopId}" : '';
        $invoiceNumber = sprintf('%s%s-%s-%04d', $prefix, $shopPrefix, $date, $sequence);
        
        // Vérifier si ce numéro existe déjà (gestion des conflits race condition)
