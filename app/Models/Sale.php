...
class Sale extends Model
{
    ...
    // Méthodes statiques
    public static function generateInvoiceNumber(?int $shopId = null): string
    {
        $prefix     = 'VTE';
        $date       = now()->format('Ymd');
        $shopPrefix = $shopId ? "S{$shopId}" : '';
        $likePrefix = "{$prefix}{$shopPrefix}-{$date}-%";

        // Verrouillage pessimiste : on lit le MAX de la séquence du jour
        // avec FOR UPDATE pour éviter deux transactions concurrentes
        // d'obtenir le même numéro.
        $maxNumber = self::whereDate('created_at', today())
            ->when($shopId, fn($q) => $q->where('shop_id', $shopId))
            ->where('invoice_number', 'like', $likePrefix)
            ->lockForUpdate()
            ->max('invoice_number');
        ...
        return sprintf('%s%s-%s-%04d', $prefix, $shopPrefix, $date, $sequence);
    }
