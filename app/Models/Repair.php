...
class Repair extends Model
{
    ...
    protected $casts = [
        ...
        // Chiffré en base pour protéger le PIN/mot de passe de l'appareil client
    ...
    public function getStatusColorAttribute(): string
    {
        return match($this->status) {
            ...
            self::STATUS_UNREPAIRABLE => 'secondary',
            default => 'light',
        };
    }
    ...
    /**
     * Calcule le coût total des pièces en exécutant une requête SQL.
     *
     * NE PAS utiliser dans une boucle / liste — préférer ->withSum('parts', 'total_cost')
     * sur la requête parente pour éviter le N+1 :
     *   Repair::withSum('parts', 'total_cost')->get()
     *   puis $repair->parts_sum_total_cost
     */
    ...
    public static function generateRepairNumber(): string
    {
        $prefix     = 'REP';
        $date       = now()->format('Ymd');
        $likePrefix = "{$prefix}-{$date}-%";

        $maxNumber = self::whereDate('created_at', today())
            ->where('repair_number', 'like', $likePrefix)
            ->lockForUpdate()
            ->max('repair_number');

        $sequence = 1;
        if ($maxNumber && preg_match('/-(\d{4})$/', $maxNumber, $matches)) {
    ...
    public function canBeDelivered(): bool
    {
        return in_array($this->status, [
            ...
            self::STATUS_UNREPAIRABLE, // Retour de l'appareil au client sans réparation
        ]);
    }
    ...
    public function updateStatus(string $status): void
    {
        ...
        if ($previous !== $status) {
            RepairStatusChanged::dispatch($this, $previous, $status);
        }
    }
}
