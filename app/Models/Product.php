...
class Product extends Model
{
    ...
    protected $fillable = [
        ...
        'wholesale_price',
        'qty_semi_wholesale_min', // Seuil qté pour prix demi-gros (défaut 3)
        'qty_wholesale_min',      // Seuil qté pour prix de gros (défaut 10)
        'quantity_in_stock',
    ...
    /**
     * Seuils de quantité par défaut (utilisés si les champs ne sont pas définis)
     */
    const QUANTITY_NORMAL_MAX = 2;
    const QUANTITY_SEMI_WHOLESALE_MIN = 3;
    const QUANTITY_SEMI_WHOLESALE_MAX = 9;
    const QUANTITY_WHOLESALE_MIN = 10;
    ...
    {
        $qtyWholesale    = (int) ($this->qty_wholesale_min      ?? self::QUANTITY_WHOLESALE_MIN);
        $qtySemiWholesale = (int) ($this->qty_semi_wholesale_min ?? self::QUANTITY_SEMI_WHOLESALE_MIN);

        if ($quantity >= $qtyWholesale && $this->wholesale_price) {
            return (float) $this->wholesale_price;
        }

        if ($quantity >= $qtySemiWholesale && $this->semi_wholesale_price) {
    ...
    /**
     * Prix minimum acceptable pour ce produit (prix de gros en priorité).
     * Le paramètre $shopId est réservé pour une future logique par boutique.
     */
    public function getMinimumPriceForShop(?int $shopId = null): ?float
    {
        $price = $this->wholesale_price ?? $this->semi_wholesale_price ?? $this->normal_price;
     ...
     * Plancher absolu : prix d'achat (ne jamais vendre à perte).
     * Plancher commercial : prix de gros comme seuil de vente minimum.
     */
    public function isPriceBelowMinimum(float $price, ?int $shopId = null): bool
    {
        // Plancher absolu : ne jamais vendre en dessous du prix d'achat
        if ($this->purchase_price > 0 && $price < (float) $this->purchase_price) {
            return true;
        }
        // Plancher commercial : prix de gros comme seuil de vente minimum
        $commercialFloor = (float) ($this->wholesale_price
            ?? $this->semi_wholesale_price
            ?? $this->normal_price
            ?? 0);
        return $price < $commercialFloor;
    }
