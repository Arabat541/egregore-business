...
class SavTicket extends Model
{
    ...
    // Générer un numéro de ticket unique par boutique
    public static function generateTicketNumber(?int $shopId = null): string
    {
        $prefix = 'SAV';
        $date   = now()->format('ymd');
        ...
        $lastTicket = $query->orderByDesc('id')->lockForUpdate()->first();

        $sequence = 1;
        if ($lastTicket && preg_match('/(\d{4})$/', $lastTicket->ticket_number, $m)) {
            $sequence = ((int) $m[1]) + 1;
        }
    ...
    {
        return $this->hasMany(SavReplacedPart::class);
    }
