...
class ProductController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'sku' => 'nullable|string|unique:products,sku,NULL,id,shop_id,' . $request->shop_id,
            ...
            'is_active' => 'boolean',
            'char_keys' => 'nullable|array',
            'char_keys.*' => 'nullable|string|max:100',
            'char_values' => 'nullable|array',
            'char_values.*' => 'nullable|string|max:255',
        ...
        // Créer le produit (sans les champs fournisseur, image et caractéristiques séparées)
        $productData = collect($validated)->except(['supplier_id', 'supplier_price', 'supplier_lead_time', 'image', 'char_keys', 'char_values'])->toArray();
        ...
        // Caractéristiques
        if ($request->filled('char_keys')) {
            $characteristics = [];
            foreach ($request->char_keys as $i => $key) {
                if (!empty(trim($key)) && isset($request->char_values[$i]) && !empty(trim($request->char_values[$i]))) {
                    $characteristics[trim($key)] = trim($request->char_values[$i]);
                }
            }
            $productData['characteristics'] = !empty($characteristics) ? $characteristics : null;
        }
    ...
    public function update(Request $request, Product $product)
    {
        $validated = $request->validate([
            'name' => 'required|string|max:255',
            'sku' => 'nullable|string|unique:products,sku,' . $product->id . ',id,shop_id,' . $request->shop_id,
            'category_id' => 'required|exists:categories,id',
            'shop_id' => 'required|exists:shops,id',
            'description' => 'nullable|string',
            'image' => 'nullable|image|mimes:jpg,jpeg,png,webp|max:2048',
            ...
            'semi_wholesale_price' => 'nullable|numeric|min:0',
            'wholesale_price'      => 'nullable|numeric|min:0',
            'qty_semi_wholesale_min' => 'nullable|integer|min:1|max:9999',
            'qty_wholesale_min'      => 'nullable|integer|min:1|max:9999',
            'stock_alert_threshold' => 'required|integer|min:0',
            'brand' => 'nullable|string|max:100',
            'model' => 'nullable|string|max:100',
            'is_active' => 'boolean',
        ...
        $updateData = collect($validated)->except(['image', 'char_keys', 'char_values'])->toArray();
        ...
        if ($request->has('char_keys')) {
            ...
            $updateData['characteristics'] = !empty($characteristics) ? $characteristics : null;
        }
