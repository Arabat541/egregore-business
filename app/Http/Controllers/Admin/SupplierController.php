...
class SupplierController extends Controller
{
    ...
    public function generateOrderPdf(Request $request)
    {
        ...
        $pdf = Pdf::loadView('admin.suppliers.order-pdf', compact(
            'supplier', 'orderItems', 'reference', 'orderDate', 'notes',
        ...
        if ($request->has('save_order')) {
            try {
                ...
                // Non-critical — PDF is still returned
            }
        }
    ...
    public function storeOrder(Request $request)
    {
        ...
        try {
            $order = $this->supplierOrderService->createOrder(
                $validated,
            ...
            return back()->withInput()->with('error', $this->handleException($e, 'Erreur lors de la création'));
        }
    }
    ...
    public function receiveOrder(Request $request, SupplierOrder $order)
    {
        ...
        try {
            $hasDiscrepancy = $this->supplierOrderService->receiveOrder(
                $order,
                $validated['items'],
            ...
            $msg = $hasDiscrepancy
                ? 'Réception enregistrée avec écarts. Vérifiez le détail des quantités reçues.'
                : 'Commande réceptionnée avec succès. Stock mis à jour.';

            return redirect()->route('admin.suppliers.orders.show', $order)->with('success', $msg);

        } catch (\Exception $e) {
            return back()->with('error', $this->handleException($e, 'Erreur lors de la réception'));
        }
    }
    ...
    public function markOrderReceived(Request $request, SupplierOrder $order)
    {
        ...
        foreach ($order->items as $item) {
            $receivedQtys[$item->id] = (int) $request->input("received.{$item->id}", $item->quantity_ordered);
            $prices[$item->id]       = (float) $request->input("prices.{$item->id}", $item->unit_price);
        }

        try {
            $this->supplierOrderService->markOrderReceived($order, $receivedQtys, $prices, Auth::id());

            return back()->with('success', 'Commande marquée comme reçue, stock et prix fournisseurs mis à jour.');

        } catch (\Exception $e) {
            return back()->with('error', $this->handleException($e, 'Erreur lors de la mise à jour'));
        }
    }
    ...
    public function storePrice(Request $request)
    {
        ...
        $this->supplierOrderService->recordSupplierPrice(
    ...
    public function quickStoreProduct(Request $request)
    {
        ...
        try {
            $product = $this->supplierOrderService->quickCreateProduct($validated, Auth::id());
            ...
            return response()->json([
                'success' => false,
                'message' => $this->handleException($e, 'Erreur lors de la création'),
            ], 500);
        }
    }
}
