...
class ExpenseController extends Controller
{
    ...
    public function exportPdf(Request $request)
    {
        ...
        $byCategory = $expenses->where('status', 'approved')
        ...
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('admin.expenses.pdf', compact(
            'expenses', 'shop', 'dateFrom', 'dateTo',
            'totalApproved', 'totalPending', 'totalAll', 'byCategory'
        ))->setPaper('a4', 'portrait');

        return $pdf->download('depenses_' . date('Y-m-d') . '.pdf');
    }
