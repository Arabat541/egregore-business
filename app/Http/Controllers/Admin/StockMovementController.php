...
class StockMovementController extends Controller
{
    ...
    /**
     * Export PDF du journal des mouvements de stock
     */
    public function exportPdf(Request $request)
    {
        ...
        $typeLabel = $request->filled('type') ? ($movementTypes[$request->type] ?? $request->type) : null;
        $totalIn   = $movements->where('quantity', '>', 0)->sum('quantity');
        $totalOut  = $movements->where('quantity', '<', 0)->sum('quantity');

        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('admin.stock-movements.pdf', compact(
            'movements', 'shop', 'dateFrom', 'dateTo', 'typeLabel', 'totalIn', 'totalOut', 'movementTypes'
        ))->setPaper('a4', 'landscape');

        return $pdf->download('mouvements_stock_' . date('Y-m-d') . '.pdf');
    }
}
