...
class ShopController extends Controller
{
    ...
    public function index()
    {
        ...
        // Caisse temps réel par boutique
        $shopCashData = [];
        foreach ($shops as $shop) {
            $openRegister = CashRegister::where('shop_id', $shop->id)
                ->where('status', 'open')
            ...
            $todayRepairsAmount = $shop->repairs()
                ->whereDate('created_at', today())
                ->selectRaw('COALESCE(SUM(labor_cost), 0) + COALESCE(SUM(parts_cost), 0) as total')
                ->value('total') ?? 0;

            $shopCashData[$shop->id] = [
                'register' => $openRegister,
                'today_sales' => $todaySalesAmount,
                'today_repairs' => $todayRepairsAmount,
                'today_ca' => $todaySalesAmount + $todayRepairsAmount,
            ];
        }

        return view('admin.shops.index', compact('shops', 'shopCashData'));
    }
    ...
    public function dashboard()
    {
        $shops = Shop::active()
            ...
            ->map(function ($shop) {
                ...
                // Ajouter le CA réparations (labor + parts) — identique à la page index()
                $todayRepairsCa = $shop->repairs()
                    ->whereDate('created_at', today())
                    ->selectRaw('COALESCE(SUM(labor_cost), 0) + COALESCE(SUM(parts_cost), 0) as total')
                    ->value('total') ?? 0;
                $shop->today_revenue += $todayRepairsCa;
                $shop->today_profit  += $todayRepairsCa;
