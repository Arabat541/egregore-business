...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        // Part du technicien sur la main d'œuvre (défaut 50%)
        $techShare = (int) Setting::get('technician_labor_share', 50) / 100;

        // CA brut du jour (main d'œuvre uniquement - part technicien, livré uniquement)
        $laborDayTotal = Repair::forTechnician($user->id)
            ->where('status', Repair::STATUS_DELIVERED)
            ->whereDate('repaired_at', today())
            ->sum('labor_cost');
        $caDayBrut = $laborDayTotal * $techShare;
        ...
        // CA brut du mois (main d'œuvre uniquement - part technicien, livré uniquement)
        $laborMonthTotal = Repair::forTechnician($user->id)
            ->where('status', Repair::STATUS_DELIVERED)
            ->whereMonth('repaired_at', now()->month)
            ->whereYear('repaired_at', now()->year)
            ->sum('labor_cost');
        $caMonthBrut = $laborMonthTotal * $techShare;
        ...
        // Réparations terminées et livrées (historique)
        $deliveredRepairs = Repair::forTechnician($user->id)
            ->whereIn('status', [Repair::STATUS_DELIVERED, Repair::STATUS_REPAIRED, Repair::STATUS_READY_FOR_PICKUP])
            ->with('customer')
            ->latest('repaired_at')
            ->take(10)
            ->get();
        ...
        $stats = [
            ...
            'tech_share_percent' => (int) Setting::get('technician_labor_share', 50),
        ];
