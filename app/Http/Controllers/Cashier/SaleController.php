...
class SaleController extends Controller
{
    ...
    /**
     * Calculer le prix correct selon la quantité et le type de client
     * C'est la même logique qu'en JavaScript pour éviter les discordances
     * 
     * RÈGLES:
     * - Clients comptoir/enregistrés: TOUJOURS prix normal (pas d'autre option)
     * - Réparateurs: JAMAIS le prix normal, uniquement prix réparateur/demi-gros/gros
     */
    private function calculateCorrectPrice(Product $product, int $quantity, string $clientType): float
    {
        // Client comptoir ou enregistré = TOUJOURS prix normal
        ...
        // 1-2 pièces = prix réparateur (obligatoire)
    ...
    public function store(Request $request)
    {
        ...
        // Vérifier le stock et les prix pour tous les produits
        foreach ($validated['items'] as $item) {
            ...
            // Pour les réparateurs, vérifier que le prix réparateur est défini
            ...
            // Calculer le prix correct selon la quantité et le type de client
            $correctPrice = $this->calculateCorrectPrice($product, $item['quantity'], $validated['client_type']);
            
            // Vérifier strictement que le prix envoyé est exact
            if ((float)$item['unit_price'] !== (float)$correctPrice) {
                return back()->with('error', "⚠️ SÉCURITÉ: Le prix du produit {$product->name} ne peut pas être modifié! Prix défini: {$correctPrice} FCFA, Prix reçu: {$item['unit_price']} FCFA. La modification des prix est interdite.");
            }
        }
        ...
        try {
            ...
            $sale = Sale::create([
                ...
                'amount_given' => $amountPaid, // Montant réellement donné par le client
