...
class ResellerPaymentController extends Controller
{
    ...
    public function createPayment(Request $request, Reseller $reseller)
    {
        ...
        // Si une facture spécifique est sélectionnée
        $selectedSale = null;
        if ($selectedSaleId) {
    ...
    {
        // Si c'est un paiement partiel de facture spécifique
    ...
    protected function storeInvoicePartialPayment(Request $request, Reseller $reseller)
    {
        ...
        // Vérifier que le montant ne dépasse pas le reste à payer de la facture
        if ($cashAmount > (float) $sale->amount_due) {
            return back()->with('error', 'Le montant (' . number_format($cashAmount, 0, ',', ' ') . ' FCFA) dépasse le reste à payer de cette facture (' . number_format($sale->amount_due, 0, ',', ' ') . ' FCFA).');
        }
        ...
        try {
            ...
            $payment = ResellerPayment::create([
                ...
                'notes' => $validated['notes'] ?? 'Paiement partiel facture ' . $sale->invoice_number,
            ]);
            ...
            $message .= 'Nouvelle dette totale: ' . number_format((float) $reseller->fresh()->current_debt, 0, ',', ' ') . ' FCFA';
