...
// Page d'accueil - Storefront (boutique en ligne)
...
// ==================== BOUTIQUE EN LIGNE (Storefront) ====================
Route::name('storefront.')->group(function () {
    // Home déjà défini comme route '/' ci-dessus
    Route::get('/catalogue', [\App\Http\Controllers\Storefront\StorefrontController::class, 'catalog'])->name('catalog');
    Route::get('/produit/{id}', [\App\Http\Controllers\Storefront\StorefrontController::class, 'product'])->name('product');

    // Panier
    Route::get('/panier', [\App\Http\Controllers\Storefront\CartController::class, 'index'])->name('cart');
    Route::post('/panier/ajouter', [\App\Http\Controllers\Storefront\CartController::class, 'add'])->name('cart.add');
    Route::patch('/panier/modifier', [\App\Http\Controllers\Storefront\CartController::class, 'update'])->name('cart.update');
    Route::delete('/panier/supprimer', [\App\Http\Controllers\Storefront\CartController::class, 'remove'])->name('cart.remove');
    Route::delete('/panier/vider', [\App\Http\Controllers\Storefront\CartController::class, 'clear'])->name('cart.clear');

    // Commande
    Route::get('/commande', [\App\Http\Controllers\Storefront\CheckoutController::class, 'index'])->name('checkout');
    Route::post('/commande', [\App\Http\Controllers\Storefront\CheckoutController::class, 'store'])->name('checkout.store');
    Route::get('/commande/confirmation/{token}', [\App\Http\Controllers\Storefront\CheckoutController::class, 'confirmation'])->name('order.confirmation');

    // Suivi commande
    Route::get('/suivi', [\App\Http\Controllers\Storefront\CheckoutController::class, 'track'])->name('track');
    Route::post('/suivi', [\App\Http\Controllers\Storefront\CheckoutController::class, 'trackOrder'])->name('track.search');
});

// ==================== 2FA Google Authenticator (accessible sans être connecté) ==
Route::prefix('2fa')->name('2fa.')->group(function () {
    ...
    Route::post('/', [\App\Http\Controllers\Auth\TwoFactorController::class, 'verify'])
        ->middleware('throttle:5,1')
        ->name('verify');
});

// ==================== RECHERCHE GLOBALE (Ctrl+K) ====================
Route::get('/search', [\App\Http\Controllers\GlobalSearchController::class, '__invoke'])
    ->middleware('auth')
    ->name('global.search');
...
Route::middleware('auth')->group(function () {
    ...
    // 2FA Google Authenticator : configuration et désactivation
...
Route::prefix('admin')
    ...
    ->group(function () {
        ...
        Route::get('/online-orders/{onlineOrder}', [\App\Http\Controllers\Admin\OnlineOrderController::class, 'show'])->name('online-orders.show');
        Route::patch('/online-orders/{onlineOrder}/status', [\App\Http\Controllers\Admin\OnlineOrderController::class, 'updateStatus'])->name('online-orders.update-status');
        Route::patch('/online-orders/{onlineOrder}/payment', [\App\Http\Controllers\Admin\OnlineOrderController::class, 'updatePayment'])->name('online-orders.update-payment');
        Route::patch('/online-orders/{onlineOrder}/note', [\App\Http\Controllers\Admin\OnlineOrderController::class, 'addNote'])->name('online-orders.add-note');
        ...
        Route::prefix('stock-movements')->name('stock-movements.')->group(function () {
            ...
            Route::get('/export-pdf', [\App\Http\Controllers\Admin\StockMovementController::class, 'exportPdf'])->name('export-pdf');
        ...
        Route::prefix('stock-transfers')->name('stock-transfers.')->group(function () {
            ...
            Route::post('/{stockTransfer}/receive', [\App\Http\Controllers\Admin\StockTransferController::class, 'confirmReception'])->name('receive');
        ...
        Route::get('/reports/sales/products-pdf', [ReportController::class, 'salesProductsPdf'])->name('reports.sales.products-pdf');
        // PDF exports
        Route::get('/reports/sales/pdf', [ReportController::class, 'salesPdf'])->name('reports.sales.pdf');
        Route::get('/reports/repairs/pdf', [ReportController::class, 'repairsPdf'])->name('reports.repairs.pdf');
        ...
        Route::get('/reports/sav/pdf', [ReportController::class, 'savPdf'])->name('reports.sav.pdf');
        // Relevé de créance revendeur (rapport global — URL distincte de accountStatement)
        Route::get('/resellers/{reseller}/report-statement', [ReportController::class, 'resellerStatement'])->name('resellers.report-statement');
        Route::get('/resellers/{reseller}/report-statement/pdf', [ReportController::class, 'resellerStatementPdf'])->name('resellers.report-statement.pdf');
        // Dépenses PDF
        Route::get('/expenses/pdf', [\App\Http\Controllers\Admin\ExpenseController::class, 'exportPdf'])->name('expenses.pdf');
        ...
        Route::prefix('cash-registers')->name('cash-registers.')->group(function () {
            ...
            Route::get('/export-pdf', [AdminCashRegisterController::class, 'exportPdf'])->name('export-pdf');
            Route::get('/{cashRegister}', [AdminCashRegisterController::class, 'show'])->name('show');
            Route::get('/{cashRegister}/pdf', [AdminCashRegisterController::class, 'showPdf'])->name('show-pdf');
...
Route::prefix('caisse')
    ...
    ->group(function () {
        ...
        // Export PDF ventes (sans caisse ouverte requise)
        Route::get('/sales/export-pdf', [SaleController::class, 'exportPdf'])->name('sales.export-pdf');
        // Modification d'une vente (sans caisse ouverte requise — correction d'erreur)
        ...
        Route::middleware(['cash.open'])->group(function () {
            ...
            Route::post('/repairs/{repair}/cancel', [CashierRepairController::class, 'cancel'])->name('repairs.cancel');
        ...
        // Transferts de stock : initiation sortante + confirmation réception entrante
        Route::get('/transferts',                      [\App\Http\Controllers\Cashier\StockTransferController::class, 'index'])->name('stock-transfers.index');
        Route::get('/transferts/initier',              [\App\Http\Controllers\Cashier\StockTransferController::class, 'create'])->name('stock-transfers.create');
        Route::post('/transferts',                     [\App\Http\Controllers\Cashier\StockTransferController::class, 'store'])->name('stock-transfers.store');
        Route::get('/transferts/mes-produits',         [\App\Http\Controllers\Cashier\StockTransferController::class, 'getMyProducts'])->name('stock-transfers.my-products');
        Route::get('/transferts/{stockTransfer}',      [\App\Http\Controllers\Cashier\StockTransferController::class, 'show'])->name('stock-transfers.show');
        Route::post('/transferts/{stockTransfer}/recevoir', [\App\Http\Controllers\Cashier\StockTransferController::class, 'confirmReception'])->name('stock-transfers.receive');
        // Aliases pour compatibilité ascendante (anciens liens /transferts-entrants)
        Route::get('/transferts-entrants',             [\App\Http\Controllers\Cashier\StockTransferController::class, 'index']);
        Route::get('/transferts-entrants/{stockTransfer}', [\App\Http\Controllers\Cashier\StockTransferController::class, 'show']);
        Route::post('/transferts-entrants/{stockTransfer}/recevoir', [\App\Http\Controllers\Cashier\StockTransferController::class, 'confirmReception']);
