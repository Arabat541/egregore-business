...
class DatabaseSeeder extends Seeder
{
    public function run(): void
    {
        ...
        // En production: créer uniquement l'admin depuis le .env
        if (app()->environment('production')) {
            $this->seedAdminFromEnv();
            $this->command->info('Compte administrateur cree depuis .env');
        } else {
            // En dev/local: créer boutique + utilisateurs de test
            $shop = $this->seedShop();
            $this->seedUsers($shop);
    ...
    protected function seedAdminFromEnv(): void
    {
        $admin = User::firstOrCreate(
            ['email' => env('ADMIN_EMAIL', 'admin@egregore.com')],
            [
    ...
    protected function seedSampleData(Shop $shop): void
    {
        ...
        if ($cat) {
            Product::firstOrCreate(['sku' => 'IP14PM'], [
                'name' => 'iPhone 14 Pro Max', 
                'category_id' => $cat->id, 
                'purchase_price' => 750000, 
                'normal_price' => 900000,           // Prix normal (1-2 pcs)
                'semi_wholesale_price' => 850000,   // Prix demi-gros (3-9 pcs)
                'wholesale_price' => 800000,        // Prix de gros (10+ pcs)
                'quantity_in_stock' => 5, 
                'brand' => 'Apple', 
                'type' => 'phone', 
                'shop_id' => $shop->id
